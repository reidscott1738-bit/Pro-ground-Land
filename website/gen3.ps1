...
$ARTICLES = @(
  ...
  @{ path='/resources/landscape-renovation-cost-baton-rouge/'; short='Renovation cost'; img='/img/reno-after.jpg';
     ...
     desc='What does a landscape renovation take in Baton Rouge? Here is what drives the scope of a front-bed renovation and where the value goes.';
     sub='Every renovation is scoped to the property. Here is what moves the scope up or down.';
     ctaH='Curious what your renovation would run?';
     sections=@(
       @{h='How big a project is it'; p=@('A landscape renovation can be a small, single-bed refresh or a full reset of every bed across the front of a house. A large property, extensive removal, or premium plant material makes it a bigger job. The only firm number comes from a look at the property, which is free.') },
       @{h='What drives the scope'; p=@('A renovation comes down to a handful of things:'); bullets=@('<strong>Bed square footage.</strong> More bed area means more prep, plants and mulch.','<strong>Plant size at install.</strong> Larger, more mature plants cost more but read as finished sooner.','<strong>Removal volume.</strong> Tearing out overgrown shrubs and hauling them off is real labor.','<strong>Access.</strong> A backyard a machine cannot reach costs more than an open front bed.','<strong>Whether irrigation is present.</strong> Existing beds and irrigation change the scope.') },
       ...
       @{h='Where the value is'; p=@('What a renovation buys is a job planned properly, built with good material, and finished so it still looks right in a few years. You get a written scope and estimate before anything starts, so there are no surprises.') })
     ...
     related=@(@{t='Landscape Renovations';u='/services/landscape-renovations/'},@{t='Best plants for clay soil';u='/resources/best-plants-for-clay-soil-south-louisiana/'},@{t='See our work';u='/gallery/'}) },
